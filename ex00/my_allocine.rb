...
requests["Find the year when the movie American Beauty released"] = "SELECT mov_year AS mov_dt_rel FROM movies WHERE mov_title = 'American Beauty';"
...
requests["Find the actors with all information who played a role in the movies 'Annie Hall'"]  = "SELECT actors.* FROM actors INNER JOIN movies_actors ON actors.id = movies_actors.act_id INNER JOIN movies ON movies_actors.mov_id = movies.id WHERE movies.mov_title = 'Annie Hall';"
